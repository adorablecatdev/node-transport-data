...
// to the KMB record's destination and gtfs_to maps to its origin, so we invert
...
// LWB routes in the GTFS feed live under the KMB brand in out/kmb/routes.json —
// see AGENCY_TO_COMPANY in transform.ts. KMB+CTB / LWB+CTB are joint routes and
// belong to the KMBCTB dataset, so they are intentionally excluded here.
const KMB_AGENCIES = new Set(["KMB", "LWB"]);
...
export async function buildKmbGtfsMapping(
  routesCsv: string,
  kmbRoutesJsonPath: string,
  mappingOutPath: string,
  fuzzyOutPath: string,
  unmappedOutPath: string,
): Promise<Record<string, MappingEntry> | undefined> {
  const kmbRoutes = await readJsonIfExists<Record<string, KmbRouteEntry>>(kmbRoutesJsonPath);
  if (!kmbRoutes) {
    console.warn(
      `[time_table] ${kmbRoutesJsonPath} missing — run \`kmb\` first if you want the GTFS mapping.`,
    );
    return undefined;
  }
  ...
  // A GTFS row's FROM/TO reflects the outbound direction of the KMB route, so
  // bound=O with service_type=1 is preferred at each tie-break.
  const byRouteOrigDest = new Map<string, Candidate[]>();
  const byRoute = new Map<string, Candidate[]>();
  for (const [key, r] of Object.entries(kmbRoutes)) {
  ...
  for (const row of rows) {
    if (!KMB_AGENCIES.has(row.agency_id)) continue;
    ...
    // Exact swapped pass — some operator-side entries are published in the
    // reverse direction from GTFS (e.g. only an outbound entry exists but its
    // origin/destination is the inverse of the GTFS row). Try FROM↔TO before
    // giving up.
  ...
  console.log(
    `[time_table] KMB GTFS mapping: exact ${exactForwardCount}, ` +
