...
import type { MappingEntry } from "./kmbctb_gtfs_mapping.js";
...
// (same physical direction as route_long_name / the mapped KMBCTB record) or 2
// (opposite direction). It does NOT correlate universally with KMBCTB API
...
// KMBCTB record_id: `KMBCTB-<route>-<bound>-<service_type>` — mid-string bound
// letter, same shape as KMB. Swap the bound (O↔I) for the mirror record.
...
export async function buildKmbctbGtfsTimetable(inputs: {
  tripsCsv: string;
  calendarCsv: string;
  ...
  // GTFS route that lands on the same KMBCTB record_id.
  ...
  console.log(
    `[kmbctb-timetable] wrote ${Object.keys(result).length} record_id timetable entries to ${inputs.outPath}`,
  );
}
