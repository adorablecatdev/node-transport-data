...
import type { MappingEntry } from "./kmb_gtfs_mapping.js";
...
// (same physical direction as route_long_name / the mapped KMB record) or 2
// (opposite direction). It does NOT correlate universally with KMB API
// bound=O/I — e.g. route 1000008's long_name describes the inbound direction
// (中港碼頭→慈雲山(北)), and its dir=1 trips physically go that same inbound
// direction, not KMB outbound. Verified against stop_times.txt terminus stops.
...
// KMB record_id: `KMB-<route>-<bound>-<service_type>`. Swap the mid-string
// bound letter (O↔I) for the mirror KMB record. Returns undefined if the
...
export async function buildKmbGtfsTimetable(inputs: {
  tripsCsv: string;
  calendarCsv: string;
  ...
  // GTFS route that lands on the same KMB record_id (service_type variants).
  ...
  console.log(
    `[kmb-timetable] wrote ${Object.keys(result).length} record_id timetable entries to ${inputs.outPath}`,
  );
}
